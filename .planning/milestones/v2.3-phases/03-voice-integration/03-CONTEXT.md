...
- **Mock 位置**: app 模块包含 MockVoiceRecognizer 实现，用于开发测试

### 接口名称
- 保持 `IVoiceRecognizer` 命名（与 Phase 2 一致）

### Mock 位置
- **app 模块**: 包含 MockVoiceRecognizer 实现
- agent-android 模块: 仅包含 IVoiceRecognizer 接口定义
- VoiceRecognizerHolder 单例由 agent-android 提供
...
- MockVoiceRecognizer: 在 app 模块（需要从 agent-android 移除）
