...
1. 宿主业务工具先注册到 Java 的 `AndroidToolManager`
2. `AndroidToolManager` 生成 `tools.json`，再通过 bridge 下发给 native
...
- `tools`
  宿主业务工具，类型是 `Map<String, ToolExecutor>`
- `channels`
...
宿主业务工具接口在 `agent-core` 的 `com.hh.agent.core.tool.ToolExecutor`。

这类工具通常不直接暴露给 native，而是先被聚合进 `call_android_tool`。

可以把它理解成：

- `ToolExecutor` 是宿主业务动作
- `AndroidToolChannelExecutor` 是顶层对外通道
...
3. `AndroidToolManager` 注册宿主业务工具和内置 channel
4. `AndroidToolManager` 聚合所有 channel 的 schema，生成 `tools.json`
5. Java 通过 `NativeMobileAgentApi` 把 `tools.json` 传给 native
6. Java 同时把 `AndroidToolManager` 注册成 `AndroidToolCallback`
7. native 侧把这些 schema 注册进自己的 tool registry
...
5. Java channel 执行自己的 `execute(...)`
6. Java 返回 `ToolResult`，再序列化成 JSON 字符串
7. native 收到结果，继续后续 agent loop
...
- 实现一个新的 `ToolExecutor`
- 在应用初始化时注册进 `Map<String, ToolExecutor>`
- 让它继续走 `call_android_tool`
