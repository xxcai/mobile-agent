...
当前仓库已经移除了 `LegacyAndroidToolChannel`、`ToolExecutor` 和 `call_android_tool` 这条 legacy 宿主业务链路。

这意味着：

- 默认暴露给 LLM 的宿主业务入口已经切到 `run_shortcut`
- 宿主业务原子能力统一通过 `ShortcutExecutor` 注入 `ShortcutRuntime`
- 新增 skill 和新文档都应围绕 `run_shortcut` / `describe_shortcut` 编写
