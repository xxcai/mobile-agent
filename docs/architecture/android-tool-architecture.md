...
2. `AndroidToolManager` 会把 `ToolExecutor` 适配进 `ShortcutRuntime`，再生成 `tools.json`
...
- `shortcutRuntime`
  宿主业务 shortcut runtime；当前通过 `ToolExecutorShortcutAdapter` 兼容已有 `ToolExecutor`
- `channels`
...
### 4. `ToolExecutor` / `ShortcutRuntime`

宿主业务工具接口在 `agent-core` 的 `com.hh.agent.core.tool.ToolExecutor`。

这类工具当前不会直接暴露给 native，而是先被适配进 `ShortcutRuntime`。
...
- [ToolExecutorShortcutAdapter.java](/Users/caixiao/Workspace/projects/mobile-agent/agent-core/src/main/java/com/hh/agent/core/shortcut/ToolExecutorShortcutAdapter.java)
...
4. 已注册的 `ToolExecutor` 会被同步适配并注册进 `ShortcutRuntime`
...
- 当前会自动通过 `ToolExecutorShortcutAdapter` 进入 `run_shortcut`
