...
public class GestureToolChannel implements AndroidToolChannelExecutor {
    ...
    public JSONObject buildToolDefinition() throws Exception {
        ...
        properties.put("observation", new JSONObject()
                .put("type", "object")
                .put("description", "基于 observation 执行时的引用信息。tap 和 swipe 都应优先使用该对象。swipe 必须用它明确指定要滚动的容器 bounds。")
                .put("properties", new JSONObject()
                        ...
                        .put("targetDescriptor", new JSONObject()
                                .put("type", "string")
                                .put("description", "目标元素或目标容器的人类可读描述，例如“发送消息按钮”或“朋友圈列表”。"))
                        .put("referencedBounds", new JSONObject()
                                .put("type", "string")
                                .put("description", "从 observation 中引用的 bounds 字符串。tap 可用它定位点击目标；swipe 必须用它指定要滚动的容器。格式如 [l,t][r,b]。"))));
        ...
        function.put("description", "执行当前宿主页面内的 Android UI 动作。tap 应先读取 android_view_context_tool，再基于最新 observation 点击目标；swipe 也应先读取 android_view_context_tool，并且必须在 observation 中明确指定要滚动的容器 bounds，运行时再在当前 Activity 内注入真实触摸事件。不要猜测裸坐标，不要在没有最新 observation 的情况下调用本工具。不要用这个通道搜索联系人、发送消息、读取剪贴板或调用宿主 App 的业务工具；这类任务应使用 call_android_tool。");
