...
import com.hh.agent.h5bench.BenchmarkForegroundActivityAwaiter;
...
import com.hh.agent.h5bench.ProductionMiniWoBRunOrchestrator;
...
public class App extends Application implements MiniWoBRunOrchestrator.Provider, MiniWoBRunOrchestrator.ExecutorProvider {
    ...
    public MiniWoBRunOrchestrator getMiniWoBRunOrchestrator() {
        ...
        BenchmarkForegroundActivityAwaiter foregroundActivityAwaiter =
                new BenchmarkForegroundActivityAwaiter(
        ...
        MiniWoBInProcessPageController.TaskHost taskHost = new MiniWoBInProcessPageController.TaskHost() {
            ...
            public void openTaskPage(String assetPath) throws Exception {
                ...
                Activity previousActivity = FloatingBallLifecycleCallbacks.getCurrentForegroundActivity();
                runOnMainThread(() -> {
                ...
                Activity stable = foregroundActivityAwaiter.awaitNewBusinessWebActivity(
                        previousActivity,
        ...
        MiniWoBBenchmarkRunner benchmarkRunner = new MiniWoBBenchmarkRunner(
                ...
                new MiniWoBAgentRunDriver(
                        NativeMobileAgentApi.getInstance(),
                        task -> MainPresenter.getInstance(ContainerActivity.SESSION_KEY)
                                .createExternalStreamMirror(buildBenchmarkConversationPrompt(task)))
        );
        MiniWoBSuiteRunner suiteRunner = new MiniWoBSuiteRunner(
                assetPath -> new MiniWoBTaskRegistry(App.this).loadSuite(assetPath),
                benchmarkRunner
        );
        return new ProductionMiniWoBRunOrchestrator(
                suiteRunner::runSuite,
                this::closeForegroundBenchmarkHost,
    ...
    private String buildBenchmarkConversationPrompt(MiniWoBTaskDefinition task) {
        return "开始 H5 基准测试任务。\n"
                + "任务ID: " + task.getTaskId() + "\n"
                + "任务说明: " + task.getInstruction();
    }
}
