...
当用户要开始、运行或重跑 H5 基准测试时，使用这个 skill。
...
- 用户只是查看 benchmark 页面说明，还没有要求启动
- 用户只是想进入 benchmark 页面，本 skill 不负责导航
- 用户要求读取单个任务页面内容，而不是启动整套 benchmark
...
- 不要绕过 `start_h5_benchmark` shortcut 去点击页面按钮。
